...
# Function to execute saq_bc.sh for given environment and trajectory length
run_saq_bc() {
    local env="$1"
    local max_traj_length="$2"
    bash scripts/sweeps/saq_bc.sh "$env" "$max_traj_length"
}
...
run_task() {
    ...
    for ((i=0; i<${#environments[@]}; ++i)); do
        run_saq_bc "${environments[i]}" "${max_traj_lengths[i]}"
    done
}
