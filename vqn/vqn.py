...
class VQN(object):
    ...
    def _both_train_step(self, rng, vqvae_train_state, qf_train_state, batch, bc=False):
        ...
        def clip_grads(grads, max_norm):
            ...
            # Calculate the coefficient for clipping
            clip_coeff = max_norm / (total_norm + 1e-6)

            # Define the function to clip the gradients
            def true_fun(g):
                # Scale down each gradient to ensure the total norm does not exceed max_norm
                return jax.tree_map(lambda x: x * clip_coeff, g)

            # Define the function to return the gradients unchanged
            def false_fun(g):
                # Return the gradients as they are
                return g

            # Conditional clipping based on whether the total norm exceeds the maximum allowed norm
